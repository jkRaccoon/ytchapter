...
export default function Guide() {
  return (
    <>
      <SEO
        title="유튜브 챕터 규칙 가이드 — 0:00 · 10초 간격 · 3개 이상"
        description="유튜브 챕터가 설명란에서 자동 인식되려면 0:00 시작, 10초 이상 간격, 3개 이상, 오름차순, 콜론 형식 5가지 규칙을 모두 만족해야 합니다."
        path="/guide"
        jsonLd={jsonLd}
      />

      <h1 className="text-2xl font-bold tracking-tight text-slate-900 md:text-3xl">유튜브 챕터 규칙 가이드</h1>
      <p className="mt-2 text-sm text-slate-600">유튜브는 설명란에 다음 규칙을 모두 만족하는 타임스탬프 목록이 있을 때만 자동으로 챕터를 활성화합니다.</p>
...
9:00 마무리 및 Q&A`}</pre>
      ...
      <p className="mt-8 text-sm text-slate-500">
        <Link to="/" className="underline">챕터 포매팅</Link>으로 돌아가기.
      </p>
    </>
  );
}
